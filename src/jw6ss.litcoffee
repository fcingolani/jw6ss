This CoffeScript / JavaScript library allows you to track JW Player 6 events using Comscore
StreamSense.
...
It just takes one argument, a *JW Player* instance, and if it isn't provided it
...
Start reporting inmediately! As the `observe()` instance method from `Reporter`
class returns `this`, this last line will return the forementioned `Reporter`
instance.

        reporter.observe()

This is the main Class for the library, it handles the mapping between player
events and *StreamSense* events, and keeps updated all the variables the latter
needs to report.

    class JW6SS.Reporter

Define some internal states as Class properties. This is needed because of how
JW fires *Seeking * events (we will get onto this later on). While just
`States.SEEKING` is really needed, we define other basic states for the sake of
completeness.
...
When a seeking event is fired, we will store its target offset in this variable.
